...
export default function UserDemandes({ data, handleClickOpenModal, setDetails }) {
    const formatDate = (d) => {
    ...
    //     })
    //         .then((response) => {
    //             fetchDemandesUtilisateur(data.user_id)
    //             handleClose()
    //             setMessage({
    //                 type: 'success',
            ...
            >
                ...
                <td className="px-6 py-4 text-center justify-center flex">
                    {data.status === 'approved' ? (
                        ...
                        <button className="p-2 bg-gray-50 border text-gray-700 rounded"
                            onClick={() => {
                                setDetails(data)
                        ...
                        >
                            {
                                loading == true ? 'Annulation...' : 'Annuler'
                            }
                        </button>
                    ) : data.status === 'canceled' ? <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" className="icon icon-tabler icons-tabler-outline icon-tabler-u-turn-left">
                        <title> demande annulée </title>

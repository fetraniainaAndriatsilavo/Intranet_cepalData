    ...
    public function store(Request $request)
    {
        try {
            ...
            if (!empty($validated['ogc_leave_id'])) {
                ...
                if (!$exists) {
                    return response()->json([
                        'message' => 'Le congé spécifié ne correspond pas à l\'utilisateur.'
                    ], 422);
                }
            }
            ...
            if ($isHoliday && strtolower($validated['type']) !== 'jour férié') {
                return response()->json([
                    'message' => 'Attention : cette date correspond à un jour férié. Vous devez sélectionner le type "Jour férié".'
                ], 422);
            }
            ...
            if ($hasLeave && strtolower($validated['type']) !== 'congé') {
                return response()->json([
                    'message' => 'Attention : vous êtes en congé à cette date. Vous devez sélectionner le type "Congé".'
                ], 422);
            }
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $validated = $request->validate([
                'user_id' => 'required|exists:intranet_extedim.users,id',
                'date' => 'sometimes|date',
                'nb_hour' => 'sometimes|min:0',
                'client_code' => 'nullable|string',
                'project_id' => 'nullable|integer|exists:intranet_extedim.projects,id',
                'type' => 'sometimes|string',
                'description' => 'nullable|string',
                'ts_period_id' => 'sometimes|exists:intranet_extedim.timesheets_periods,id',
                'status' => 'nullable|string',
                'ogc_leave_id' => 'nullable|integer',
                'approved_by' => 'nullable|integer',
                'approved_at' => 'nullable|date',
            ...
            return response()->json([
                'message' => 'Timesheet introuvable.',
                'error' => $e->getMessage()
            ], 404);
        } catch (\Throwable $e) {
            Log::error('Erreur lors de la mise à jour du timesheet : ' . $e->getMessage(), [
                'stack' => $e->getTraceAsString()
            ]);
